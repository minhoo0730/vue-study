            ...
            <ul>
              <li v-for="(menu, mIdx) in addMenuList" :key="mIdx" class="py-2" style="border-bottom:1px solid #e1e1e1;">
                <div class="d-flex justify-space-between align-center mb-2" style="">
                  <p> {{ menu.name }}</p>
                  <div class="">
                    <button class="btn minus-btn" @click.prevent="onMinusInquiry(menu)" :disabled="menu.inQuiry == 1">-</button>
                    <input type="number" min="1" class="text-center" style="width:50px;" v-model.number="menu.inQuiry" readonly/>
                    <button class="btn plus-btn" @click.prevent="onPlusInquiry(menu)">+</button>
                  </div>
                </div>
          ...
          <div class="d-flex flex-column justify-space-between" style="height:140px; border-top:1px solid #e9e9e9;">
            <div class="pa-4">
              <p class="mb-2" style="font-size:2.0rem;">
                주문수량<span><b>{{ totalMenuCounting.inQuirySum }}</b></span
                >개
              </p>
              <p style="font-size:2.0rem;">
                합계
                <span><b>{{ priceToString(totalMenuCounting.priceSum) }}</b>원</span>
              </p>
            </div>
...
<script setup>
  ...
  // 주문내역에 담겨있는 메뉴의 개수와 가격을 모두 계산한다.
  const totalMenuCounting = computed(() => {
    const totalCountData = {
      inQuirySum:0,
      priceSum:0
    };
    if(addMenuList.value.length > 0){
      addMenuList.value.forEach(data => {
        totalCountData.inQuirySum = totalCountData.inQuirySum + data.inQuiry;
        totalCountData.priceSum = totalCountData.priceSum + data.addPrice;
      })
